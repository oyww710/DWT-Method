...
PValue<-tF.normal.P(40,40,0,1,0,0.4,10000)

write.csv(PValue,"Power_Norm_40_40_0_1_0_0.4.csv")
#setwd ("D:/DWT")
#PValue<- read.csv(file="Power_Norm_40_0_1_0.4_0.csv",head=TRUE,sep=",")

length(which(PValue[,1]<0.05))/dim(PValue)[1]

length(which(PValue[,2]<0.05))/dim(PValue)[1]

length(which(PValue[,3]<0.05))/dim(PValue)[1]


length(which(PValue[,4]<0.05))/dim(PValue)[1]

length(which(PValue[,9]<0.05))/dim(PValue)[1]

length(which(PValue[,10]<0.05))/dim(PValue)[1]
length(which(PValue[,14]<0.05))/dim(PValue)[1]

length(which(PValue[,15]<0.05))/dim(PValue)[1]
minp<-function(p1, p2)
{
...
abf=1-sqrt(1-0.05); #Bonferroni 
	pwr=length(which( minp(PValue[,1], PValue[,6])<abf ))/10000;
pwr
